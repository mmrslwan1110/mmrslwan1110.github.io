...
### 对比度
img4=contrast(send_image)
crop_im3 = cv2.cvtColor(np.array(img4), cv2.COLOR_RGBA2BGRA)
cv2.imwrite("contrast.jpg", crop_im3)

### 锐化
img=sharpen(send_image)
crop_im = cv2.cvtColor(np.array(img), cv2.COLOR_RGBA2BGRA)
cv2.imwrite("sharpen.jpg", crop_im)
...
img=flip(send_image)
crop_im = cv2.cvtColor(np.array(img), cv2.COLOR_RGBA2BGRA)
cv2.imwrite("flip.jpg", crop_im)

### 裁剪

img=crop(send_image)
crop_im = cv2.cvtColor(np.array(img), cv2.COLOR_RGBA2BGRA)
cv2.imwrite("crop.jpg", crop_im)

### 拉伸

img=deform(send_image)
crop_im = cv2.cvtColor(np.array(img), cv2.COLOR_RGBA2BGRA)
cv2.imwrite("deform.jpg", crop_im)



### 镜头畸变

img=distortion(send_image)
crop_im = cv2.cvtColor(np.array(img), cv2.COLOR_RGBA2BGRA)
cv2.imwrite("distortion.jpg", crop_im)

### 椒盐噪声

img=addNoise(send_image)
crop_im = cv2.cvtColor(np.array(img), cv2.COLOR_RGBA2BGRA)
cv2.imwrite("addNoise.jpg", crop_im)
...
crop_im = cv2.cvtColor(np.array(img), cv2.COLOR_RGBA2BGRA)
cv2.imwrite("vignetting.jpg", crop_im)

### 抠图
img=cutout(send_image)
crop_im = cv2.cvtColor(np.array(img), cv2.COLOR_RGBA2BGRA)
cv2.imwrite("cutout.jpg", crop_im)
